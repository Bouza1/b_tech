...
const AnimatedImage = ({ src, alt, title, bannerStats }: Props) => {
    ...
    return (
        <motion.div key={key}
             ...
             viewport={{ once: false }}
        >
            <p className="font-palanquin text-3xl sm:text-4xl text-gray-600 font-bold text-center">{title?.substring(0, title?.length - 2)}<span
                className="text-xl">{title?.substring(title?.length - 2, title?.length)}</span></p>
            ...
            <motion.div className="flex flex-row gap-1 md:w-full justify-evenly"
                initial={{
                    opacity: 0.4,
                    x: 0,
                    y: 0,
